...
先別做。先問我這幾題，一次問一題。
我答得含糊或答不出來的時候，不要照抄我的話就往下走，
用你知道的追問一輪，幫我把它想清楚：
1. 這場講給誰聽？他們現在對這件事知道多少？
2. 他聽完要做什麼？（一個具體的動作，不是「了解一下」）
...
https://raw.githubusercontent.com/yazelin/slide-deck-skill/HEAD/templates/deck.css
   整份的顏色都吃 deck.css 最上面 :root 那組變數。
   我如果要換風格或換色，就只改那組值，不要去改個別規則。
   配色的部分：這場如果是在講一個看得到的東西（某個網站、某個 app、某個品牌），
   就去取它自己的顏色來用，不要另外配一組。取不到或這場沒有這種對象，
   直接用模板預設往下做，不用為了顏色停下來問我。
   做完在 spec.md 那段寫一行色是從哪裡取的，我要換再跟你說。

二、依我上面的答案決定要留哪些功能、砍哪些。
   例如會議室提案用不到手機遙控與預錄備援影片，就別放進來。

三、把下面這些寫成 spec.md，放在簡報旁邊：
   （1）四題的答案，用我自己的話
   （2）你依這些答案做的每一個取捨：留了什麼、砍了什麼、各是為了滿足哪一題
   （3）你認為這份簡報要通過的驗收標準
   之後我要改稿或換工具，先看那份，不用重新判斷一次。
```

開頭那句主題不能省。這份發包單實測過一次：沒寫主題的話，AI 會先反問你要講什麼，
多花一輪。四題本身不用改，那四題才是一般人不會自己想到的。

第三點常被跳過，但那是你唯一帶得走的東西。**四題是這裡給的，答案是你的**——答完如果只留在跟 AI 的對話裡，關掉視窗就沒了。而且只存答案還不夠：取捨的理由跟驗收標準沒記下來，下次換一個 AI，它得重新判斷一次，也可能判斷成別的樣子。`spec.md` 三段都寫，下次就是把那份丟過去。

配色的規則是**取，不是挑**。這場簡報如果是在講一個看得到的東西，那個東西自己已經有顏色了，把它的色票拿過來，整份簡報就跟主角是同一套視覺，台下不用被迫在兩種風格之間切換。要講的是自家產品，就取產品站的顏色；要講一個開源專案，就取它官網或 README 的主視覺色；要講的東西有品牌規範，取品牌規範。

**沒有這種對象的場合很多**，講觀念、講方法、講一份調查結果都是，那就用模板預設的琥珀配米白，這條規則對你不成立，不用勉強生一個出來。

取到的值只填 `templates/deck.css` 最上面 `:root` 那 17 個變數，底色、字色、主色、次色、淺色頁的四個色都在裡面，其他規則一行都不用動。刻意不吃變數的例外只有五種，換色時它們本來就不該跟著變：陰影與遮罩的黑、圖片與縮圖的底、iframe 與 QR 的白底（QR 白底變色會掃不到）、假瀏覽器視窗列的中性灰。這幾處在 `deck.css` 頂端標了註解。

**取完把來源記下來**，寫進 `spec.md` 或 `deck.css` 頂端一行註解都行，不然下個月回頭看沒有人知道這組色哪來的。

（實際跑過兩次：一次講的是一個許願池網站，就取它設計系統裡的琥珀金加青，淺色頁沿用它自己的淺色主題；另一次講的東西沒有獨立的設計系統，退一階取了品牌規範的顏色。）
